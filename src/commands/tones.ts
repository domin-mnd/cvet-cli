...
import { tones } from "cvet";
import type { HEX } from "cvet/types";
import { color } from "@oclif/color";

export default class Tones extends Command {
  static description = "Generate an array of tones with a provided quantity.";
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(Tones);
    ...
    const colors = tones(args.hex as HEX, +flags.quantity);
